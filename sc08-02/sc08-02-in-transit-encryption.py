...
#
# TASK 2 / SC-8, SC-8(1), SC-13 ΓÇö ENCRYPTION IN TRANSIT
#
# OCI TOOLING:
...
def source_selfcheck() -> bool:
    ...
    for node in ast.walk(tree):
        ...
        if name in PSK_READS and getattr(node, "lineno", -1) not in allowed:
            print(f"SELF-CHECK: FAILED ΓÇö {name} at line {node.lineno} would read an "
                  "IPSec pre-shared key", file=sys.stderr)
            return False
    for name in PSK_READS:
        if name in SDK_READ_METHODS:
            print(f"SELF-CHECK: FAILED ΓÇö {name} is in the allowlist", file=sys.stderr)
            return False
    return True
