...
ln -s "$ROOT/sc08-02/tests/mock-oci-task2" "$TMP/bin/oci"
...
PATH="$TMP/bin:$PATH" MOCK_SINGLE_TUNNEL=1 bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" \
  -c ocid1.compartment.oc1..test \
  -r us-langley-1 \
  -s ipsec \
...
set +e
PATH="$TMP/bin:$PATH" MOCK_DENY_TUNNEL=1 bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" \
  -c ocid1.compartment.oc1..test \
  -r us-langley-1 \
  -s ipsec \
...
set +e
PATH="$TMP/bin:$PATH" MOCK_BAD_SHAPE_LB=1 bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" \
...
PATH="$TMP/bin:$PATH" MOCK_FORMULA_NAME=1 bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" \
  -c ocid1.compartment.oc1..test -r us-langley-1 -s lb \
...
PATH="$TMP/bin:$PATH" MOCK_BACKEND_VERIFY_FALSE=1 bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" \
  -c ocid1.compartment.oc1..test -r us-langley-1 -s lb \
...
PATH="$TMP/bin:$PATH" MOCK_VOLUME_FIELD_MISSING=1 bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" \
