...
python3 - "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" <<'PY'
import re
import shlex
import sys
...
for fixture in mutation secret; do
  mkdir -p "$TMP/$fixture/lib" "$TMP/$fixture/sc08-02"
  cp "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" "$TMP/$fixture/sc08-02/sc08-02-in-transit-encryption.sh"
  cp "$ROOT/lib/oci-scope-selector.sh" "$TMP/$fixture/lib/oci-scope-selector.sh"
done

sed -i '0,/iam compartment list/s//iam compartment delete/' "$TMP/mutation/sc08-02/sc08-02-in-transit-encryption.sh"
set +e
bash "$TMP/mutation/sc08-02/sc08-02-in-transit-encryption.sh" --selfcheck > "$TMP/mutation.out" 2>&1
mutation_rc=$?
set -e
[ "$mutation_rc" -eq 1 ]
grep -q 'SELF-CHECK: FAILED' "$TMP/mutation.out"

sed -i '0,/iam compartment list/s//network ip-sec-psk get/' "$TMP/secret/sc08-02/sc08-02-in-transit-encryption.sh"
set +e
bash "$TMP/secret/sc08-02/sc08-02-in-transit-encryption.sh" --selfcheck > "$TMP/secret.out" 2>&1
secret_rc=$?
set -e
[ "$secret_rc" -eq 1 ]
...
PATH="$TMP/bin:$PATH" MOCK_SCOPE_LOG="$TMP/unknown-service.log" \
  bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" -c ocid1.compartment.oc1..vcn \
...
PATH="$TMP/bin:$PATH" MOCK_SCOPE_LOG="$TMP/tenancy-c.log" \
  bash "$ROOT/sc08-02/sc08-02-in-transit-encryption.sh" -c ocid1.tenancy.oc1..scope \
