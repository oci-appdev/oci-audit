...
mkdir -p "$TEST_TMP/injected/lib"
cp "$SCRIPT" "$TEST_TMP/injected/cm02-01-configuration-baseline.sh"
ln -s "$ROOT/cm08-hw-sw-baseline.sh" "$TEST_TMP/injected/cm08-hw-sw-baseline.sh"
ln -s "$ROOT/lib/oci-scope-selector.sh" "$TEST_TMP/injected/lib/oci-scope-selector.sh"
ln -s "$ROOT/lib/cm02-01-reconcile.py" "$TEST_TMP/injected/lib/cm02-01-reconcile.py"
printf '\noci compute instance terminate --instance-id ocid1.instance.example\n' \
  >> "$TEST_TMP/injected/cm02-01-configuration-baseline.sh"
set +e
bash "$TEST_TMP/injected/cm02-01-configuration-baseline.sh" --selfcheck \
