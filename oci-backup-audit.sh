...
echo "compartment_id,service,resource,backup_configured,frequency_schedule,retention,detail" > "$OUT"

# CSV-safe append (quote fields, escape embedded quotes)
row() {
  local IFS=','
  local out=""
  for f in "$@"; do
...
echo "======================================================================"
TOTAL="$(($(wc -l < "$OUT") - 1))"
UNPROT="$(awk -F',' 'NR>1 && ($4=="\"NO\"" || $4=="\"UNKNOWN\"")' "$OUT")"
...
if [ "$UNPROT_COUNT" -gt 0 ]; then
  ...
  printf '%s\n' "$UNPROT" | awk -F',' '{gsub(/"/,"",$2); gsub(/"/,"",$3); gsub(/"/,"",$4); printf "  [%-14s] %-40s (%s)\n", $2, $3, $4}'
fi
echo
