...
SCRIPT="$ROOT/cm08-01/cm08-01-component-inventory-baseline.sh"
...
mkdir -p "$TMP/injected/lib" "$TMP/injected/cm08-01" "$TMP/injected/cm02-01"
cp "$SCRIPT" "$TMP/injected/cm08-01/cm08-01-component-inventory-baseline.sh"
cp "$ROOT/cm08-01/cm08-hw-sw-baseline.sh" "$TMP/injected/cm08-01/cm08-hw-sw-baseline.sh"
ln -s "$ROOT/lib/oci-scope-selector.sh" "$TMP/injected/lib/oci-scope-selector.sh"
ln -s "$ROOT/cm08-01/cm08-01-reconcile.py" "$TMP/injected/cm08-01/cm08-01-reconcile.py"
ln -s "$ROOT/cm02-01/cm02-01-reconcile.py" "$TMP/injected/cm02-01/cm02-01-reconcile.py"
printf '\nemit unsafe.csv unsafe "." compute instance terminate --instance-id ocid1.instance.example\n' \
  >> "$TMP/injected/cm08-01/cm08-hw-sw-baseline.sh"
set +e
bash "$TMP/injected/cm08-01/cm08-01-component-inventory-baseline.sh" --selfcheck \
...
printf '\noci_discover unsafe compute instance terminate --instance-id ocid1.instance.example\n' \
  >> "$TMP/injected/cm08-01/cm08-01-component-inventory-baseline.sh"
set +e
bash "$TMP/injected/cm08-01/cm08-01-component-inventory-baseline.sh" --selfcheck \
...
PATH="$TMP/bin:$PATH" MOCK_TASK8_LOG="$TMP/direct-engine.log" \
  bash "$ROOT/cm08-01/cm08-hw-sw-baseline.sh" -c "$COMP" -r us-langley-1 \
