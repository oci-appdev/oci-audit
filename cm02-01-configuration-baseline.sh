...
POSTPROCESSOR="$SCRIPT_DIR/lib/cm02-01-reconcile.py"
...
readonly_selfcheck() {                                           # selfcheck-exempt
  ...
  deny='(oci|oci_q|oci_capture)[[:space:]].*(create|update|delete|change|move|restore|enable|disable|rotate|assign|attach|detach|terminate|reboot|import|export|upload|push|install|remove|refresh|run-now|promote|switch)([[:space:]]|$)' # selfcheck-exempt
