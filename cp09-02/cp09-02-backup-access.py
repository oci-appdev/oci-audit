...
#
# TASK 1 / CP-9, CP-9(5), AC-3, AC-6 ΓÇö WHO CAN REACH AND DESTROY THE BACKUPS
#
# OCI TOOLING:
...
def source_selfcheck() -> bool:
    ...
    for node in ast.walk(tree):
        ...
        if name in ("get_preauthenticated_request", "access_uri") \
            ...
            print(f"SELF-CHECK: FAILED ΓÇö {name} at line {node.lineno} would put a "
                  "bearer URL into evidence", file=sys.stderr)
            return False
    if "get_preauthenticated_request" in SDK_READ_METHODS:
        print("SELF-CHECK: FAILED ΓÇö bearer-URL read is in the allowlist",
              file=sys.stderr)
        return False
    return True
