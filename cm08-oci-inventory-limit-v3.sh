#!/usr/bin/env bash
#==============================================================================
# oci_limits_quota_audit.sh                                       v1.2.0
#
# READ-ONLY audit of OCI service limits, resource availability, and quota
# policies. Mirrors the Console view:
#   Governance & Administration > Limits, Quotas and Usage
#
# Outputs (CSV, one set per run, per region):
#   1. limits_usage_<region>_<ts>.csv   description / service limit / used /
...
#   * available + used does NOT always equal service_limit. Where a quota
#     policy applies, the service computes availability against the effective
#     quota, not the service limit. This build reports pct_used relative to
#     service_limit only (effective-quota-value is not emitted), so where a
#     quota caps a resource below its service limit, pct_used UNDERSTATES
#     consumption against the binding cap. The EXHAUSTED status still fires
#     correctly in that case, because it is driven by available <= 0 rather
#     than by pct_used. Cross-reference quota_policies_<ts>.csv when a limit
#     shows EXHAUSTED at a low pct_used.
#   * Limits are per-region. AD-scoped limits are emitted once per AD.
#
# CHANGE LOG
#   v1.2.0  Removed column: eligible_for_increase. Moved description from last
#           position to column 3 (immediately after service_name). Output
#           schema is 11 columns (was 12). summary.txt parsers switched to
#           end-relative field addressing -- see note above the Summary block.
#   v1.1.0  Removed columns: fractional_usage, fractional_availability,
#           effective_quota_value, quotas_supported. pct_used base changed
#           from effective_quota_value (fallback service_limit) to
#           service_limit only. Output schema is 12 columns (was 16).
...
# Reads a TAB-delimited task on $1..$7, emits one TAB-delimited result row
# of 8 fields: region, svc, limit, scope, ad, service_limit, used, available.
#------------------------------------------------------------------------------
fetch_availability() {
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$region" "$svc" "$lname" "$scope" "$ad" "$limval" "$used" "$avail"
}
...
for REGION in "${REGIONS[@]}"; do
  ...
  # TASKS schema (8 TAB fields):
  #   1 region  2 service  3 limit_name  4 scope_type  5 availability_domain
  #   6 service_limit  7 is-resource-availability-supported  8 description
  ...
  {
    echo 'region,service_name,description,limit_name,scope_type,availability_domain,service_limit,used,available,pct_used,status'

    # Rejoin TASKS field 8 (description) onto the 8-field RESULTS rows,
    # keyed on region+service+limit+scope+ad -> 9 TAB fields.
    awk -F'\t' -v OFS='\t' 'NR==FNR{key=$1 SUBSEP $2 SUBSEP $3 SUBSEP $4 SUBSEP $5; meta[key]=$8; next}
    ...
    | awk -F'\t' -v W="$WARN_PCT" -v C="$CRIT_PCT" '
      function esc(s) { gsub(/"/,"\"\"",s); return "\"" s "\"" }
      {
        ...
        print esc(region) "," esc(svc) "," esc(desc) "," esc(lname) "," esc(scope) "," \
              esc(ad) "," esc(lim) "," esc(used) "," esc(avail) "," esc(pct) "," esc(status)
      }'
  } > "$OUT_CSV"
...
#------------------------------------------------------------------------------
# Summary
#   Column map for the CSVs parsed below:
#     1 region  2 service_name  3 description  4 limit_name  5 scope_type
#     6 availability_domain  7 service_limit  8 used  9 available
#     10 pct_used  11 status
#
#   The awk parsers below split on '","' and address every field EXCEPT
#   description from the END of the record ($NF, $(NF-1), ...). description now
#   sits at column 3 rather than last, and a description containing the literal
#   sequence  ","  would otherwise introduce spurious field boundaries and shift
#   every column to its right. End-relative addressing keeps status and pct_used
#   correct regardless, since any such corruption is confined to the left of
#   them. Rows with NF < 11 are skipped rather than misread.
#------------------------------------------------------------------------------
SUMMARY="${OUT_DIR}/summary_${TS}.txt"
{
  ...
  echo "pct_used base   : service_limit (effective quota value not collected)"
  echo
  for f in "${LIMITS_CSVS[@]}"; do
    [[ -f "$f" ]] || continue
    echo "--- $(basename "$f") ---"
    awk -F'","' 'NR>1 && NF>=11 {s=$NF; gsub(/"/,"",s); c[s]++}
                 END{for (k in c) printf "  %-16s %6d\n", k, c[k]}' "$f"
    echo
    echo "  Top utilization:"
    awk -F'","' 'NR>1 && NF>=11 {
                   pct=$(NF-1); ad=$(NF-5); lname=$(NF-7); svc=$2;
                   gsub(/"/,"",pct); gsub(/"/,"",ad); gsub(/"/,"",lname); gsub(/"/,"",svc);
                   if (pct != "") printf "%8.2f  %s / %s %s\n", pct, svc, lname, ad }' "$f" \
