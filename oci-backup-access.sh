...
echo "compartment_id,policy_name,statement,grantee,backup_resource_matched" > "$POL_OUT"
echo "compartment_id,bucket,public_access,par_count,par_details,versioning,retention_rules,risk" > "$OBJ_OUT"

csvrow() {  # $1 = target file; remaining = fields
  local file="$1"; shift
  local out=""
  for f in "$@"; do
    f="${f//\"/\"\"}"
...
if [ -n "$SINGLE_COMP" ]; then
  ...
  COMPS="$(o iam compartment list --compartment-id-in-subtree true \
            --access-level ANY --lifecycle-state ACTIVE --all \
            --query 'data[].id' --raw-output 2>/dev/null | jq -r '.[]' 2>/dev/null)"
  [ -n "$TENANCY_ID" ] && COMPS="$TENANCY_ID"$'\n'"$COMPS"
fi
COMP_COUNT="$(printf '%s\n' "$COMPS" | grep -c . || true)"
...
RISK_PUBLIC="$(awk -F',' 'NR>1 && $3!="\"NoPublicAccess\""' "$OBJ_OUT" | grep -c . || true)"
RISK_PAR="$(awk -F',' 'NR>1 && $4!="\"0\"" && $4!="\"\""' "$OBJ_OUT" | grep -c . || true)"
...
if [ "$RISK_PUBLIC" -gt 0 ] || [ "$RISK_PAR" -gt 0 ]; then
  ...
  awk -F',' 'NR>1 {gsub(/"/,"",$2); gsub(/"/,"",$8); if($8!="ok") printf "  bucket %-40s risk: %s\n", $2, $8}' "$OBJ_OUT"
  echo
fi
