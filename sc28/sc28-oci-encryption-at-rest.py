...
#
# TASK 3 / SC-28, SC-28(1), SC-12 ΓÇö ENCRYPTION AT REST
#
# OCI TOOLING:
...
# COLLECTION-FAILED row plus a non-OK coverage row and exit 3 ΓÇö never an empty
...
# "OCI" does not ΓÇö it is an OCI Vault key, which is customer-managed but not
...
def source_selfcheck() -> bool:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Attribute) and any(node.attr.startswith(p) for p in banned):
            print(f"READ-ONLY SDK SELF-CHECK: FAILED ΓÇö mutating call {node.attr}",
                  file=sys.stderr)
            return False
    return True
