...
**Outstanding implementation item — Tasks 1, 2 and 3:** the CP-9, SC-8 and
SC-28 collectors are not yet retrofitted to the strict automation contract in
`SCRIPT-DESIGN-STANDARD.md`. Their default no-scope-flag path is fully guarded,
but their explicit `-c`/`-n` paths run without the double-OCID confirmation and
without exact `--approve-scan YES`. `Implementation complete` for these three
tasks therefore covers evidence collection, not the current scope-automation
contract. Retrofit before scheduling any of the five scripts as unattended
automation.
