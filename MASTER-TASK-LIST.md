...
| 6 | Ports/protocols/services list, approval proof and restricted list | Partial | `cm07-01-open-ports-protocols-services.sh` has the scope-confirmation, evidence-separation and reconciliation foundation, whose cross-compartment coverage and ICMP restricted-match defects are now fixed; the legacy `cm07-openports.sh` produced useful output while `cm07-ppsm.sh` and `cm07-proof-opened-ports.sh` did not; see `CM07-CORRECTIVE-REVIEW.md` | **Code complete 2026-09-02** — cross-compartment resolution, portless ICMP matching, `-p/--profile`, semantic rule identity and the packaged scan summary are implemented with regressions (`tests/test-cm07-01-corrective.sh`). Remaining: the controlled known-object compartment and tenancy runs, count reconciliation against Console/CLI and reviewer disposition, then collect approval/restricted-list evidence |
